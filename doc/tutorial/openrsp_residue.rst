...
:c:func:`OpenRSPGetRSPFun`. Users can therefore refer to :ref:`slide-response-function`
for the explanation of the arguments with the same names, or
...
respectively equal to the ``order_residue`` excitation energies.

The excitation energies will be in ``excit_energy``, corresponding to
``order_residue`` excitations (we say an "excitation tuple"). Considering
...
(or an excitation tupe ``(r1,s1)``) that two (sums of) frequencies of
perturbation labels will equal to their excitation energies.

Now if we have other two excited states ``(r2,s2)`` and ``(r3,s3)``
that will be used for double residue calculations, we could set
``num_excit=3`` for the number of excitation tuples used in calculations.
