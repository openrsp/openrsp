...
The choice of appropriate :math:`(k,n)` rule usually affect the efficiency
of calculations. Detailed discussion of the :math:`(k,n)` rule can be found,
for instance in:
...
The ``size_rsp_funs`` equals to the sum of the size of each property
to calculate. In this example, if we use non-redundant representation of
...
Further considering the number of frequency configurations of
:math:`\alpha` (2) and :math:`\beta` (3), we have ``size_rsp_funs``
as
...
setting the ``ref_ham``, ``ref_state`` and ``ref_overlap``, which are
``struct QcMat*`` in C and ``type(QcMat)`` in Fortran. They are respectively
the Hamiltonian (Fock matrix), atomic orbital based density matrix and
