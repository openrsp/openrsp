...
module module_interest_osr
  ...
  integer, public, parameter :: lmx1 = 6 
  integer, public, parameter :: lmx2 = 2*lmx1+1 
  integer, public, parameter :: lmx4 = 4*lmx1+1 
  integer, public, parameter :: ncc(lmx1) = (/1,3,6,10,15,21/)
  integer, public, parameter :: nsc(lmx1) = (/1,3,5, 7, 9,11/)
...
! ---------------------------------------------------------------------------------------
  subroutine interest_osr_class_nuclear(li1,li2,fint,cosr,t,                                 &
    ...
    if( mmax.eq.1 )then
      li1=1 
      li2=1 
      fint(1) = cosr*fnx(1)      !todo: zvaz ci cosr*fnx(1), alebo iba fnx(1) 
      return
    endif

    do m=1,mmax 
      vaux(1,m) = cosr*fnx(m)    !todo: zvaz ci cosr*fnx(m), alebo iba fnx(m) 
    enddo
    ...
    write(6,*) ' expected:  <1-13>'
    write(6,*) ' required: ',lab
    write(6,*) ' Abnormal termination in module: interest_osr, class: nuclear, routine: main'
    stop
  end subroutine
