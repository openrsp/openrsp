...
!           res(1:(nr_atoms*3)**len(pert)) = 0.0d0
            ...
            xc_dmat = 0.0d0
            call dcopy(mat_dim*mat_dim, D%elms_0, 1, xc_dmat(1), 1)
            do j = 1, 3
               call dcopy(mat_dim*mat_dim, Df(j)%elms_0, 1, xc_dmat(mat_dim*mat_dim*2 + 1), 1)
               do i = 1, 1
                  element = i &
                          + (j-1)*nr_atoms*3
                  call xc_integrate(           &
...
!     ---------------------------------------------------------------------------
      ...
      xc_dmat = 0.0d0
      do imat = 1, nr_dmat
         call daxpy(mat_dim*mat_dim, 1.0d0, D(imat)%elms_0, 1, xc_dmat((imat-1)*mat_dim*mat_dim + 1), 1)
      end do
      ...
      if (present(Fg)) then
         do i = 1, nr_atoms*3
            ...
            call daxpy(mat_dim*mat_dim, 1.0d0, xc_fmat, 1, Fg(i)%elms_0, 1)
         end do
      end if
      ...
      if (present(Fgg)) then
         do i = 1, nr_atoms*3
            do j = 1, i
               ...
               call daxpy(mat_dim*mat_dim, 1.0d0, xc_fmat, 1, Fgg(i, j)%elms_0, 1)
            end do
         end do
      end if
